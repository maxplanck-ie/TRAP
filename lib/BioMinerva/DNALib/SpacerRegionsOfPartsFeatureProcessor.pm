...
package BioMinerva::DNALib::SpacerRegionsOfPartsFeatureProcessor;
...
sub create {
  return new BioMinerva::DNALib::SpacerRegionsOfPartsFeatureProcessor();
}
...
sub process_feature
{
  ...
  if( @{ $part_features } ) {
    my @spacer_sequences =
      map {
        ( $_->'type' ne 'subfeature' )
          ? ( $_->'sequence' )
          : ()
      }
      @{ $feature->unified_subsequences_and_spacer_sequence_elements(
        $part_features
      ) };
    
      
    my $spacer_features = [];
    foreach my $spacer_sequence( @{ $spacer_sequences } ) {
      my $spacer_feature = new BioMinerva::DNALib::Feature();
      $spacer_feature->set_type( 'region' );
      $spacer_feature->set_sequence( $spacer_sequence );
      $target_feature->add_part_feature(  $spacer_feature );
    }
  }
