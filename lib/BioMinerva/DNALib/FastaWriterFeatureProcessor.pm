...
package BioMinerva::DNALib::FastaWriterFeatureProcessor;
...
sub create {
  return new BioMinerva::DNALib::FastaWriterFeatureProcessor();
}
...
#      if( !$feature->contains( $subsequence ) ) {
 #       confess( "Splicing problem: $feature does not contain $subsequence (overlap is not enough!)" );
  #    }
  ...
  {
    my $feature_attrs = $feature->attrs() || {};
    foreach my $attr_key( keys %{ $feature_attrs } ) {
      $attrs->{ $attr_key } =
        join( ",", map { defined( $_ ) ? $_ : "undef" } 
          @{ $feature_attrs->{ $attr_key } || [] } 
        );
    }
  }

  $attrs->{ 'primary_sequence_id' } = $feature->get_primary_sequence_id();
  $attrs->{ 'start' } = $feature->get_start();
  $attrs->{ 'end' } = $feature->get_end();
  if( defined( $feature->get_strand() ) ) {
  ...
  if( defined( $feature->get_source() ) ) {
    $attrs->{ 'source' } = $feature->get_source();
  }
  if( defined( $feature->get_type() ) ) {
    $attrs->{ 'type' } = $feature->get_type();
  }
  if( defined( $feature->get_score() ) ) {
    $attrs->{ 'score' } = $feature->get_score();
  }
  if( defined( $feature->get_source() ) ) {
    $attrs->{ 'source' } = $feature->get_source();
  }
  
  $attrs->{ 'location_str' } = $feature->location_str();
  ...
  ### ----- final writing -----
  $self->_write_fasta( $seq, "$id\t$attrs_str" );
}
