...
hits2Rdat=${TRAPPATH}/utilities/hits2Rdat.R
splitFile=${TRAPPATH}/utilities/splitFile.pl 

#########################
flags="-g 0.5 --ttype balanced"
dir=hits.$$
out=hits
proc=1
keep=0
...
usage() {
      ...
      echo "3. optional: flags to control ANNOTATE [-f <ANNOTATE-flags>]. Default: $flags "
      echo "4. optional: number of processors [-p <int>]. Split set of motif matrices into \$proc chunks. Default: $proc "
      echo "5. optional: name of output directory [default: -d hits.\$\$ ]"
      echo "6. optional: name of output matrix [default: -o hits.\$\$.Rdat ]"
      echo "7. optional: flag to keep output from ANNOTATE (GFF file): -k"
      echo ""
      echo "Output directory: "
      echo "1. log                        log-file with run time parameters"
      echo "2. two matrices (Rdat files): hits full dataframe (\$out.DF) and simple sequences-motif matrix (\$out.Rdat)"
      ...
      echo ""
      echo "Usage:"
      echo "$0 -s <sequence file> -m <matrix file>  [-f ANNOTATE flags] [-o <Rdat file>]"
      echo ""
      echo "Example:" 
      echo "$0 -s test.fa -m test.pssm -d hits_dir"
      echo ""
...
do  
    ...
    gfftmp=$gdir/$gfftmp
    echo "$annotate -s $seq --pssm $m $flags > $gfftmp &" >> $log 
    $annotate -s $seq --pssm $m $flags > $gfftmp &
done
...
echo "collecting ANNOTATE results: cat $gdir/*gff > $gff" >> $log
cat $gdir/*gff > $gff


# convert gff into Rdat file and matrices
echo "convert $gff to Rdat: $Rscript $hits2Rdat $gff $out" >> $log
$Rscript $hits2Rdat $gff $out


# remove original ANNOTATE output (if not to be kept)
if [ $keep == "0" ];
then
