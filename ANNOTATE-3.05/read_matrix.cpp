...
//================================================================
void read_matrix_file(const char *mat_file_name){
...
void matrix_set_parameters(){
	...
	//discard
	if(score_mat_forw.matrix_params.count("discard") > 0){
		DISCARD_POS = true;
		...
		discard_pos_param = score_mat_forw.matrix_params["discard"];
...
void adapt_matrix_params(){
	...
	if (EXTN_THRESH){
		...
		if (!thresh_type.compare("balanced")){
			if (!BALANCED) {
				cerr<<"ERROR: Balanced cutoff selected, but not provided in the matrix file"<<endl;
				cerr<<"Invalid options. Check manual for proper usage with -h option."<<endl;
				exit(1);
			}
			score_mat_forw.score_cutoff = score_mat_forw.score_cutoff_balanced;
		} else if (!thresh_type.compare("typeI")){
			if (!TYPEI) {
				cerr<<"ERROR: TYPEI cutoff selected, but not provided in the matrix file"<<endl;
				cerr<<"Invalid options. Check manual for proper usage with -h option."<<endl;
				exit(1);
			}
			score_mat_forw.score_cutoff = score_mat_forw.score_cutoff_I;
		} else if (!thresh_type.compare("typeII")){
			if (!TYPEII) {
				cerr<<"ERROR: TYPEII cutoff selected, but not provided in the matrix file"<<endl;
				cerr<<"Invalid options. Check manual for proper usage with -h option."<<endl;
				exit(1);
			}
			score_mat_forw.score_cutoff = score_mat_forw.score_cutoff_II;
		}
